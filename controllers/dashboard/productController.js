...
class productController {
    ...
    add_product = async (req, res) => {
        ...
        form.parse(req, async (err, field, files) => {
            let { name, category, description, description2, stock, price, discount, shopName, brand } = field;
            let { images } = files;
            name = name.trim()
    ...
    product_update = async (req, res) => {
        let { name, description, description2, stock, price, discount, brand, productId, category } = req.body;
        name = name.trim()
        const slug = name.split(' ').join('-')

        try {
            await productModel.findByIdAndUpdate(productId, {
                name, description, description2, stock, price, discount, brand, productId, slug, category
            })
            const product = await productModel.findById(productId)
