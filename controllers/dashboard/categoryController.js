    ...
    add_category = async (req, res) => {
        ...
        form.parse(req, async (err, fields, files) => {
            if (err) {
                ...
                try {
                    const result = await cloudinary.uploader.upload(image.filepath, { folder: "music_categorys" })
                    if (result) {
                        const category = await categoryModel.create({
                            name, slug, image: result.url
                        })
                        responseReturn(res, 201, { category, success: "category pige kala" })
                    } else {
                        responseReturn(res, 404, { error: "malakiaaaa sta image" })
                    }
                } catch (error) {
                    // console.log(error);

                    responseReturn(res, 500, { error: "o server ta epaixe" })
                }
